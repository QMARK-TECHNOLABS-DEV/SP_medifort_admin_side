    ...
    <div className="p-4 grid grid-cols-1  sm:grid-cols-2  gap-6 md:grid-cols-3 pb-60">
      {galleryItems.map(item => (
        <div key={item.id} className="relative " >
          ...
          />
          <FiEdit className='absolute top-12 right-12 w-8  bg-[#FFFFFF] text-primaryColor bg-opacity-90 p-2 rounded-full text-sm flex items-center mr-2' size={35} />
          <div className="absolute inset-x-0 bottom-0 text-left pl-6  flex flex-col justify-end p-2  ">
            
            <h3 className="text-customRed text-xs pl-3  pb-2 ">{item.title}</h3>
            <h2 className="text-primaryColor text-md pl-3  pb-2 ">{item.name}</h2>
