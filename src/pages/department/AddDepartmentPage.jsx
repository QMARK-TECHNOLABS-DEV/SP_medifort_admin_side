...
const AddDepartmentPage = () => {
    ...
    const breadcrumbsItems = [
        ...
        { label: isEdit ? "Update Department" : "New Department", href: "/add-department" },
    ];
    ...
    const handleProcedureChange = (type, headingIndex, sectionIndex, subSectionIndex, event) => {
        const newProcedures = [...treatmentAndProcedures];
        if (type === "heading") {
            newProcedures[headingIndex].heading = event.target.value;
        } else if (type === "section") {
            newProcedures[headingIndex].sections[sectionIndex].section = event.target.value;
        } else if (type === "subSection") {
            newProcedures[headingIndex].sections[sectionIndex].subSections[subSectionIndex] = event.target.value;
        }
        setTreatmentAndProcedures(newProcedures);
    };
    ...
    const handleRemoveHeading = (headingIndex) => {
        const newProcedures = treatmentAndProcedures.filter((_, index) => index !== headingIndex);
        setTreatmentAndProcedures(newProcedures);
    };

    const handleRemoveSection = (headingIndex, sectionIndex) => {
        const newProcedures = [...treatmentAndProcedures];
        newProcedures[headingIndex].sections = newProcedures[headingIndex].sections.filter((_, index) => index !== sectionIndex);
        setTreatmentAndProcedures(newProcedures);
    };

    const handleRemoveSubSection = (headingIndex, sectionIndex, subSectionIndex) => {
        const newProcedures = [...treatmentAndProcedures];
        newProcedures[headingIndex].sections[sectionIndex].subSections = newProcedures[headingIndex].sections[sectionIndex].subSections.filter((_, index) => index !== subSectionIndex);
        setTreatmentAndProcedures(newProcedures);
    };
                                    ...
                                    <div key={headingIndex} className="mb-4">
                                        <div className="flex items-center gap-2">
                                            <input
                                                ...
                                                value={item.heading}
                                                onChange={(e) => handleProcedureChange("heading", headingIndex, null, null, e)}
                                            />
                                            <button
                                                type="button"
                                            ...
                                            <div key={sectionIndex} className="ml-4 mb-2">
                                                <div className="flex items-center gap-2">
                                                    <input
                                                        ...
                                                        onChange={(e) => handleProcedureChange("section", headingIndex, sectionIndex, null, e)}
                                                    />
                                                    <button
                                                        type="button"
                                                        className="text-red-600"
                                                        onClick={() => handleRemoveSection(headingIndex, sectionIndex)}
                                                    >
                                                        <FaTrashAlt />
                                                    </button>
                                                </div>
                                                    ...
                                                    <div key={subSectionIndex} className="ml-8 mb-2">
                                                        <div className="flex items-center gap-2">
                                                            <input
                                                                ...
                                                                value={subSection}
                                                                onChange={(e) => handleProcedureChange("subSection", headingIndex, sectionIndex, subSectionIndex, e)}
                                                            />
                                                            <button
                                                                type="button"
                                                                className="text-red-600"
                                                                onClick={() => handleRemoveSubSection(headingIndex, sectionIndex, subSectionIndex)}
                                                            >
                                                                <FaTrashAlt />
                                                            </button>
                                                        </div>
                                                ...
                                                <button
                                                    ...
                                                    onClick={() => handleAddSubSection(headingIndex, sectionIndex)}
                                                >
                                                    + add sub section
                                                </button>
                                            </div>
